...
const handleSaveMap = async () => {
    ...
    try {
      ...
      // 5. Navigate back to Create Event Step 4
      navigate('/create-event', {
        state: {
          targetStep: 4,
          seatMapId: savedData.seatMapId,
          seatMapMongoId: savedData.seatMapMongoId,
...
>
  {/* Outer container: Gives 80px space around all 4 edges so top is never flush */}
  <div 
  ...
  >
    {/* Scaled bounding wrapper: Expands the scrollable area as zoom increases */}
    <div 
      style={{
    ...
    >
      {/* Canvas Board: Always scales from top-left, so it NEVER expands into negative Y coordinates */}
      <div 
        ...
        style={{ 
          ...
          transformOrigin: 'top left' /* <--- CRITICAL FIX */
        }}
      >
        {showGrid && viewMode === 'creator' && <div className={`${canvasGridBg} print:hidden`} style={{ display: viewMode === 'preview' ? 'none' : 'block' }}>
      ...
      {viewMode === 'creator' && (
        <div className="flex items-center space-x-4 print:hidden">
         <button 
            ...
            disabled={isEmpty || isSavingMap}
            className={`font-bold px-4 py-2 rounded-md shadow transition text-xs ${
              isEmpty || isSavingMap ? 'bg-slate-300 text-slate-500 cursor-not-allowed' : 'bg-blue-700 hover:bg-blue-800 text-white cursor-pointer'
            }`}
          >
            {isSavingMap ? 'Generating & Saving...' : 'Save Map'}
          </button>
