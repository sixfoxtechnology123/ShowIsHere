...
const handleSaveMap = async () => {
    ...
    try {
      // 1. Temporarily clear selections and active selection boxes for a clean snapshot
      setSelectedSectionId(null);
      setSelectedShapeId(null);
      setSelectedRowKey(null);
      setSelectedSeatKey(null);
      ...
      // 4. Send to the backend endpoint
      const res = await API.post('/seatmaps/save', payload);
      const savedData = res.data || res;

      hasInteractedRef.current = false;
      alert(`Seat map successfully saved as ${savedData.seatMapId || 'SM'}!`);

      // 5. Navigate back to Create Event Step 4, passing the generated SM ID and screenshot
      navigate('/create-event', {
        state: {
          targetStep: 4,
          seatMapId: savedData.seatMapId, // Passes "SM1", "SM2", etc.
