...
const handleSaveMap = async () => {
  try {
    ...
    if (boardElement) {
      // Temporarily clear any pan/zoom transform so the snapshot is perfectly centered and crisp
      const originalTransform = boardElement.style.transform;
      boardElement.style.transform = 'none';

      // Export directly at 600 x 750 resolution
      previewImageData = await toPng(boardElement, {
        cacheBust: true,
        quality: 0.95,
    ...
    const payload = {
      name: layoutName || 'Auditorium Layout',
    ...
    console.error('Error saving seat map snapshot:', error);
  }
};
