...
import {
  ...
  artistCirclePlaceholderText,ri
  artistDescImageGrid,
  accountFooterInner,
  accountSecondaryBtn,
  accountPrimaryBtn,
          ...
          {viewMode === 'form' ? (
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <button
                    ...
                    className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold px-4 py-1.5 rounded-md transition cursor-pointer border border-slate-200 shadow-xs flex items-center space-x-1"
                  >
                    <span>📋 View List</span>
              ...
              {entryMode === 'excel' && !editId ? (
                <div className="py-8 px-4 flex flex-col items-center justify-center border-2 border-dashed border-slate-300 rounded-md bg-slate-50 my-4">
              ...
              <div
                ...
                className={`${artistCirclePickerContainer} relative group cursor-pointer overflow-hidden`}
              >
                {photoBase64 ? (
             ...
             {/* Fixed Footer Bar matching CreateEvent */}
              <footer className="bg-white border-t border-slate-200 fixed bottom-0 left-0 right-0 z-40 shadow-lg w-full h-14 flex items-center">
                <div className={accountFooterInner}>
                  ...
                  >
                    <span>{isSaving ? 'Saving...' : entryMode === 'excel' && !editId ? 'Import & Save Excel' : editId ? 'Update Artist' : 'Save Artist'}</span>
                  </button>
                </div>
              </footer>
            </form>
            ...
            <div className="space-y-4">
              <div className={artistListHeaderRow + " flex items-center justify-between"}>
                <div className="flex items-center space-x-2">
                  <select
                    value={filterType}
                    onChange={(e) => setFilterType(e.target.value)}
                    className="bg-slate-50 border border-slate-300 rounded-md px-2.5 py-1.5 text-xs font-medium focus:outline-none"
                  >
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <button
                    ...
                    className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-3 py-1.5 rounded-md transition cursor-pointer border border-blue-600 shadow-xs flex items-center space-x-1"
                  >
                    <span>+ New Add</span>
                  </button>
                </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700">
                    {loading ? (
                      <tr>
                        <td colSpan="6" className="text-center py-4 text-slate-400 text-xs">Loading Artists...</td>
                      </tr>
                      ...
                      filteredData.map((item, index) => (
                        <tr key={item._id} className={artistTableRowStyle}>
                          ...
                          <td className="py-2 px-2 font-semibold">{item.artistName || 'N/A'}</td>
                          <td className="py-2 px-2">{item.artistType}</td>
                          <td className="py-2 px-2 truncate max-w-xs">{item.description || 'N/A'}</td>
                      ...
                      <tr>
                        <td colSpan="6" className="text-center py-6 text-slate-400 text-xs">No artists found.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
