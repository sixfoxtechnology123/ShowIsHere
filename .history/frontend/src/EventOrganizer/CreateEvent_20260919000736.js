...
const extractEmbedSrcFromLink = (link) => {
  ...
  // 1. Direct coordinates format like "22.5726,88.3639"
  if (/^-?\d+(\.\d+)?,\s*-?\d+(\.\d+)?$/.test(trimmed)) {
    return `https://maps.google.com/maps?q=${encodeURIComponent(trimmed)}&z=17&output=embed`;
  }

  // 2. Short links (goo.gl / maps.app.goo.gl) - embed via Google search query
  ...
  try {
    ...
    if (qParam) {
      return `https://maps.google.com/maps?q=${encodeURIComponent(qParam)}&z=17&output=embed`;
    }

    // 4. URLs with /place/Name/@lat,lng,zoom format
...
<iframe
  ...
  loading="lazy"
  src={(() => {
    // 1. Direct GPS / Google Map link handling
    if (formData.venueGoogleMapLink && !formData.venueGoogleMapLink.includes('app.goo.gl')) {
      return `https://maps.google.com/maps?q=${encodeURIComponent(formData.venueGoogleMapLink)}&z=17&output=embed`;
    }

    // 2. Build progressive search query
    const queryParts = [
    ...
    // 3. Dynamic Zoom Level:
    // - Full address + pin/city: z=17 (exact location)
    // - Name + address: z=14 (area level)
    // - Name only (e.g., 'SBI'): z=11 (wide city/region view showing all red pins)
    let zoomLevel = 11;
    ...
    return `https://maps.google.com/maps?q=${encodeURIComponent(query)}&z=${zoomLevel}&output=embed`;
  })()}
></iframe>
