...
<div className="space-y-6">
  {/* Top Section: Compact 600x750 Scaled Box + Right Controls */}
  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
    
    {/* Left Column: Compact preview preserving 600x750 aspect ratio */}
    <div className="lg:col-span-6 flex flex-col items-center">
      
      {/* Zoom Controls (Outside the box) */}
      {seatMapImage && (
        <div className="w-full max-w-[380px] flex items-center justify-between pb-1.5 px-1">
      ...
      {/* Box strictly scaled to 600x750 proportion without blowing up page size */}
      <div
        ...
        className={`w-full max-w-[380px] aspect-[600/750] border-2 border-slate-200 rounded-2xl relative flex items-center justify-center overflow-hidden transition bg-slate-100 shadow-sm ${
          seatMapImage
            ? isDraggingImg
      ...
      >
        {seatMapImage ? (
          ...
          <div className="p-6 text-center text-slate-400">
            ...
            <span className="text-xs font-semibold block text-slate-600">600 × 750 Seating Blueprint</span>
            <span className="text-[11px] text-slate-400 mt-1 block">Drag & drop image or use builder</span>
          </div>
        )}
      </div>
    </div>

    {/* Right Column: Buttons & Notice */}
    <div className="lg:col-span-6 flex flex-col gap-3 justify-center pt-2 lg:pt-8">
      <Link 
        to="/seatmap" 
      ...
      <div className="p-3 bg-blue-50/60 border border-blue-100 rounded-xl flex items-start gap-2.5 mt-2">
        ...
        <p className="text-[11px] text-slate-600 leading-relaxed">
          Upload an image of your event seat map as per your ticket category (.jpg or .png 600 × 750px recommended)
        </p>
      </div>
    </div>
  </div>

  {/* SECTION FOOTER (Located at the bottom of the page content, completely outside the image box) */}
  {seatStats && (
    <div className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col md:flex-row items-center justify-between gap-4 mt-6">
      {/* Tallies */}
      <div className="flex items-center space-x-6">
        <div>
          <span className="text-[10px] text-slate-400 uppercase font-semibold block leading-tight">Total Seats</span>
          <span className="font-extrabold text-slate-900 text-base">{seatStats.totalSeats || 0}</span>
        </div>
        <div className="h-6 w-[1px] bg-slate-200" />
        <div>
          <span className="text-[10px] text-slate-400 uppercase font-semibold block leading-tight">Available</span>
          <span className="font-extrabold text-emerald-600 text-base">{seatStats.availableSeats || 0}</span>
        </div>
        <div className="h-6 w-[1px] bg-slate-200" />
        <div>
          <span className="text-[10px] text-slate-400 uppercase font-semibold block leading-tight">Reserved</span>
          <span className="font-extrabold text-amber-600 text-base">{seatStats.reservedSeats || 0}</span>
        </div>
      </div>

      {/* Category Counts (VIP, Regular, etc.) */}
      {seatStats.categoryCounts && Object.keys(seatStats.categoryCounts).length > 0 && (
        <div className="flex items-center flex-wrap gap-2 md:border-l md:border-slate-200 md:pl-4">
          {Object.entries(seatStats.categoryCounts).map(([catName, count]) => (
            <span
              key={catName}
              className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold bg-white text-slate-700 border border-slate-200 shadow-2xs"
            >
              <span className="w-2 h-2 rounded-full bg-blue-600 mr-1.5" />
              {catName}: <strong className="ml-1 text-slate-900">{count}</strong>
            </span>
          ))}
        </div>
      )}
