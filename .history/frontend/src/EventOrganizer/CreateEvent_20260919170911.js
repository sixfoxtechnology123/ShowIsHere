...
<div className="flex flex-col xl:flex-row gap-8 items-start justify-center">
  {/* Left: Fixed 600px width unit containing 600x750 Box & Its Image Footer */}
  <div className="w-[600px] shrink-0 flex flex-col items-center">
    
    {/* ZOOM CONTROLS (OUTSIDE TOP) */}
    {seatMapImage && (
      <div className="w-[600px] flex items-center justify-between pb-2 px-1">
        <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
          Layout Canvas (600 × 750 px)
        </span>
    ...
    {/* THE MAIN 600 x 750 FRAME WITH ATTACHED IMAGE FOOTER */}
    <div className="w-[600px] border-2 border-slate-300 rounded-2xl overflow-hidden bg-white shadow-md flex flex-col shrink-0">
      
      {/* EXACT 600x750 IMAGE VIEWPORT */}
      <div
      ...
      >
        {seatMapImage ? (
          ...
          <div className="p-8 text-center text-slate-400">
            ...
            <span className="text-sm font-semibold block text-slate-700">600 × 750 px Seating Canvas</span>
            <span className="text-xs text-slate-400 mt-1 block">Drag & drop image or use seat map creator</span>
          </div>
        )}
      </div>

      {/* IMAGE FOOTER: SHOWS TOTAL, AVAILABLE, RESERVED, AND CATEGORY COUNTS */}
      {seatStats ? (
        <div className="w-[600px] bg-white border-t border-slate-200 px-4 py-3 flex flex-col gap-2.5">
          {/* Main Counter Stats */}
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-6">
              <div className="text-left">
                <span className="font-extrabold text-slate-900 text-base block leading-none">{seatStats.totalSeats || 0}</span>
                <span className="text-[10px] text-slate-500 font-medium uppercase mt-0.5 block">Total Seats</span>
              </div>
              <div className="h-6 w-[1px] bg-slate-200" />
              <div className="text-left">
                <span className="font-extrabold text-emerald-600 text-base block leading-none">{seatStats.availableSeats || 0}</span>
                <span className="text-[10px] text-slate-500 font-medium uppercase mt-0.5 block">Available</span>
              </div>
              <div className="h-6 w-[1px] bg-slate-200" />
              <div className="text-left">
                <span className="font-extrabold text-amber-600 text-base block leading-none">{seatStats.reservedSeats || 0}</span>
                <span className="text-[10px] text-slate-500 font-medium uppercase mt-0.5 block">Reserved</span>
              </div>
            </div>
          </div>

          {/* Dynamic Categories (VIP, Regular, etc.) */}
        ...
        <div className="w-[600px] bg-slate-50 border-t border-slate-200 py-2.5 px-4 text-xs text-slate-400 text-center font-medium">
