...
<div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
  <div className="lg:col-span-7 space-y-3">
    {/* EXACT h-48 BOX WITH INTEGRATED ZOOM & PAN */}
    <div
    ...
    >
      {seatMapImage ? (
        <>
          ...
          <div 
            className="absolute top-2 right-2 z-20 flex items-center gap-1 bg-white/90 backdrop-blur-xs px-1.5 py-1 rounded-md shadow-xs border border-slate-200 text-xs font-bold"
            onMouseDown={(e) => e.stopPropagation()}
          >
            <button
              type="button"
              onClick={() => setImgZoom((prev) => Math.max(0.6, Number((prev - 0.2).toFixed(2))))}
              className="w-5 h-5 flex items-center justify-center rounded hover:bg-slate-200 cursor-pointer text-slate-700 font-extrabold"
              title="Zoom Out"
            >
              -
            </button>
            ...
            <button
              ...
              className="px-1 text-[10px] hover:bg-slate-200 rounded cursor-pointer text-slate-600"
              title="Reset"
            >
              Reset
            </button>
          ...
          {/* Draggable and Scalable Seat Map Image */}
          <img
            src={seatMapImage}
            alt="Seat Map"
            draggable={false}
        ...
        <span className="text-xs text-slate-400 font-medium pointer-events-none">
          Drag & drop seating image here or use the buttons on the right
        </span>
      )}
    </div>
