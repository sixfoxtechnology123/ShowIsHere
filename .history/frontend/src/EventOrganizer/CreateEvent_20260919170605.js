...
useEffect(() => {
  ...
  if (location.state?.seatMapId) {
    setSelectedSeatMapId(location.state.seatMapId); // Receives "SM1", "SM2", etc.
  }
}, [location.state]);
...
<div className="space-y-2 w-full">
  ...
  {/* Left Column (7 Cols): Seat Map Image Preview with Given Dimensions */}
  <div className="lg:col-span-7 w-full">
    {seatMapImage ? (
      <div className="w-full space-y-2">
        <div className="flex justify-between items-center bg-slate-100 px-3 py-2 rounded-lg border border-slate-200">
          <div className="flex items-center space-x-2">
            <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></span>
            <span className="text-xs font-bold text-slate-800">
              Seat Map Snapshot {selectedSeatMapId && <span className="text-blue-600 font-mono">({selectedSeatMapId})</span>}
            </span>
          </div>
          <div className="flex items-center space-x-3">
            <Link
              to="/seatmap"
              className="text-xs text-blue-600 font-bold hover:underline cursor-pointer no-underline"
            >
              Edit Layout
            </Link>
        ...
        {/* Scaled Preview Frame: maintains aspect ratio and fits cleanly */}
        <div className="w-full h-[480px] rounded-2xl overflow-hidden border border-slate-300 bg-white flex items-center justify-center p-3 shadow-inner">
          <img
            src={seatMapImage}
            alt="Seat Map Preview"
            className="w-full h-full object-contain rounded-lg drop-shadow-sm transition-transform duration-200 hover:scale-[1.02]"
          />
        </div>
      </div>
    ) : (
      /* Empty State Placeholder */
      <div className="w-full h-[480px] border-2 border-dashed border-slate-300 rounded-2xl flex flex-col items-center justify-center bg-slate-50 p-6 text-center">
        <div className="w-14 h-14 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center mb-3 shadow-xs">
        ...
        <h4 className="text-sm font-bold text-slate-800 mb-1">No Seat Map Added</h4>
        <p className="text-xs text-slate-500 max-w-sm mb-4">
          Create a layout using the builder to automatically generate a snapshot, or upload an existing seating blueprint image.
        </p>
        <Link
          to="/seatmap"
          className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold no-underline shadow-xs transition cursor-pointer"
        >
          Create Seat Map
        </Link>
      </div>
  ...
  {/* Right Column (5 Cols): Controls & Action Buttons */}
  <div className="lg:col-span-5 flex flex-col gap-3 justify-center pt-2 lg:pt-6">
    <Link 
      to="/seatmap" 
      className="py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg text-center no-underline shadow-xs transition cursor-pointer flex items-center justify-center space-x-2"
    >
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-4 h-4">
        <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
      </svg>
      <span>{seatMapImage ? 'Open in Seat Map Creator' : 'Create Seat Map'}</span>
    </Link>
    
    <div className="text-center text-xs text-slate-400 font-medium my-1">or</div>

    <label className="py-3 px-4 bg-white border-2 border-slate-200 hover:border-slate-300 text-slate-800 text-xs font-semibold rounded-lg text-center cursor-pointer shadow-xs transition flex items-center justify-center space-x-2">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-4 h-4 text-slate-500">
        <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5" />
      </svg>
      <span>Upload Seating Blueprint Image</span>
      <input 
        type="file" 
        accept="image/*" 
        onChange={(e) => {
