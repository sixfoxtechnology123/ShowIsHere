                ...
                <div className="mt-5 flex items-center ">
                  ...
                  {hasMultiple && (
                    <button 
                      ...
                      className="w-8 h-8 rounded-lg  text-rose-500  flex items-center justify-center font-bold text-sm cursor-pointer transition"
                      title="Delete slot"
                    >
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                  ...
                  {isLast && (
                    <button 
                      ...
                      className="w-8 h-8 rounded-lg  text-blue-600 flex items-center justify-center font-bold text-base cursor-pointer transition"
                      title="Add time slot"
                    >
                      +
                    </button>
...
<div className="space-y-5 pt-2 w-full">
      ...
      <input 
        ...
        onChange={(e) => {
          ...
          if (currentList.length === 0) {
            setFormData({ 
              ...formData, 
              selectedWeeklyDates: [val], 
              weeklyTimeSlots: [{ date: val, startTime: '', endTime: '' }] 
            });
            e.target.value = '';
          ...
          if (diffDays < 0) {
            ...
            const updatedSlots = formData.sameTimeSlotForAll 
              ...
              : [...(formData.weeklyTimeSlots || []), { date: val, startTime: '', endTime: '' }];
  ...
  {/* Selected Dates Pills: Cross icon only appears on HOVER */}
        ...
        {formData.selectedWeeklyDates.map((dateStr, i) => {
          ...
          return (
            <div key={i} className="relative group px-4 py-2 bg-blue-600 text-white rounded-md text-xs font-bold shadow-xs inline-flex items-center justify-center">
              <span>{formatted}</span>
              {/* Cross icon visible strictly when hovering over the pill */}
              <button
                type="button"
                onClick={() => {
                ...
                className="ml-2 w-4 h-4 rounded-full bg-white text-slate-800 flex items-center justify-center text-[11px] font-bold cursor-pointer hover:bg-rose-100 hover:text-rose-600 opacity-0 group-hover:opacity-100 transition-opacity"
                title="Remove date"
              >
                ×
              </button>
  ...
  <div className="space-y-4 w-full pt-2">
    ...
    {/* CASE 1: All days share the same slots */}
    {formData.sameTimeSlotForAll ? (
        ...
        {(formData.weeklyTimeSlots || [{ date: 'all', startTime: '', endTime: '' }]).map((slot, index, arr) => {
          ...
          return (
            <div key={index} className="flex items-center gap-3 w-full">
              ...
              {/* Action Buttons: Delete on all rows if > 1, '+' on the last row */}
      ...
      /* CASE 2: Individual date-wise card boxes */
      (formData.selectedWeeklyDates || []).map((dateVal) => {
        ...
        // Get slots specific to this date along with their global index
        ...
        return (
          <div key={dateVal} className="border border-slate-200 rounded-xl p-4 bg-white shadow-2xs space-y-3">
            {/* Header: Date title rendered once + whole date card deletion */}
              ...
              {dateSlotsWithIndex.map((slotItem, localIndex) => {
                ...
                return (
                  <div key={slotItem.globalIndex} className="flex items-center gap-3 w-full">
                    ...
                    {/* Row Buttons:
                        - If only 1 row: shows ONLY '+' on that row.
                        - If > 1 row: shows Delete on ALL rows, and '+' on the last row. */}
                    <div className="mt-5 flex items-center gap-1.5">
                      {hasMultipleRows && (
                        <button 
                          type="button" 
