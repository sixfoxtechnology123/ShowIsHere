...
<iframe
  ...
  loading="lazy"
  src={(() => {
    // 1. If user provided any Google Map Link or GPS coords, parse and show it
    if (formData.venueGoogleMapLink?.trim()) {
      return extractEmbedSrcFromLink(formData.venueGoogleMapLink);
    }

    // 2. Otherwise, build progressive query from input fields
    const queryParts = [
    ...
    // Name only ('SBI'): z=11 (shows multiple red pins)
    // Name + Area: z=14 (area view)
    // PIN / Full Address: z=17 (exact branch)
    let zoomLevel = 11;
