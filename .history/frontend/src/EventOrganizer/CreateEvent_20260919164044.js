...
<div className="space-y-2 w-full">
  ...
  {/* Left Column (7 Cols): Seat Map Preview or Empty Box */}
  <div className="lg:col-span-7 w-full">
    {selectedSeatMapId ? (
      <div className="w-full space-y-2">
        <div className="flex justify-between items-center bg-slate-100 px-3 py-2 rounded-lg border border-slate-200">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs font-bold text-slate-800">
              Interactive Layout: <span className="text-blue-600 font-mono">{selectedSeatMapId}</span>
            </span>
          </div>
          <button
            type="button"
        ...
        {/* Interactive canvas preview with zoom & drag */}
        <SeatMapViewer seatMapId={selectedSeatMapId} />
      </div>
    ) : seatMapImage ? (
      /* Fallback: Uploaded Static Image Preview */
      <div className="w-full space-y-2">
        <div className="flex justify-between items-center bg-slate-100 px-3 py-2 rounded-lg border border-slate-200">
          <span className="text-xs font-bold text-slate-700">Uploaded Image Preview</span>
          <button
            type="button"
            onClick={() => setSeatMapImage(null)}
            className="text-xs text-rose-600 font-bold hover:underline cursor-pointer"
          >
            Remove Image
          </button>
        </div>
        <div className="w-full h-[520px] rounded-2xl overflow-hidden border border-slate-300 bg-slate-900 flex items-center justify-center p-2">
          <img 
            src={seatMapImage} 
            alt="Uploaded Seat Map Preview" 
      ...
      <div className="w-full h-[520px] border-2 border-dashed border-slate-300 rounded-2xl flex flex-col items-center justify-center bg-slate-50/50 p-6 text-center">
        ...
        <h4 className="text-sm font-bold text-slate-800 mb-1">No Seat Map Added Yet</h4>
        <p className="text-xs text-slate-500 max-w-sm">
          Design custom sections, rows, and categories using the interactive builder, or upload an existing seating blueprint image.
        </p>
      </div>
    )}
  </div>

  {/* Right Column (5 Cols): Action Buttons */}
  <div className="lg:col-span-5 flex flex-col gap-3 justify-center pt-2 lg:pt-8">
    <Link 
      to="/seatmap" 
    ...
    >
      ...
      <span>{selectedSeatMapId ? 'Edit in Seat Map Creator' : 'Open Seat Map Creator'}</span>
    </Link>
    ...
    <label className="py-3 px-4 bg-white border-2 border-slate-200 hover:border-slate-300 text-slate-800 text-xs font-semibold rounded-lg text-center cursor-pointer shadow-xs transition flex items-center justify-center space-x-2">
      ...
      <span>Upload Seating Image</span>
      <input 
        type="file" 
        accept="image/*" 
        onChange={(e) => {
