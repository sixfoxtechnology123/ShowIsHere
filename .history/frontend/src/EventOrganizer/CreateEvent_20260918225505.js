...
// 1. Helper function to gather all inputs safely
  ...
  // 2. Real API call when clicking "Save as Draft"
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (savedDoc?._id) setCreatedEventId(savedDoc._id);

      setIsDataSaved(true);
      const eventCode = savedDoc?.createEventId ? ` (${savedDoc.createEventId})` : '';
      toast.success(res.data?.message || `Draft saved${eventCode}!`);
    } catch (err) {
  ...
  // 3. Real API call when clicking Next / Submit
  const handleProceed = async () => {
    ...
    try {
      ...
      if (activeStep < steps.length) {
        // Steps 1-5: Save progress to database and advance step
        const res = await API.post('/events/save-step', payload);
        const savedDoc = res.data?.data || res.data || res;
        if (savedDoc?._id) setCreatedEventId(savedDoc._id);

        const eventCode = savedDoc?.createEventId ? ` (${savedDoc.createEventId})` : '';
        toast.success(`Step ${activeStep} saved${eventCode}!`, { duration: 1500 });
        ...
        const publishedDoc = res.data?.data || res.data;
        const eventCode = publishedDoc?.createEventId ? ` [${publishedDoc.createEventId}]` : '';
        toast.success(res.data?.message || `Event published successfully${eventCode}!`);
      }
    } catch (err) {
      console.error('Proceed step error:', err);
