...
<div className="pt-6 border-t border-slate-100 space-y-4 w-full">
  ...
  <div className="flex flex-col gap-6 w-full">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input 
            ...
            onChange={(e) => {
              // Allows only letter characters, spaces, and hyphens (Blocks numbers and venue names)
              const val = e.target.value;
        ...
        <div>
          ...
          <input 
            ...
            onChange={(e) => {
              // Strictly digits only, max 6 digits. Completely prevents searching venue names here.
    ...
    {/* Clean Map Container (No white place box, direct scroll enabled) */}
        ...
        {formData.venueName?.trim() || formData.venueAddress?.trim() || formData.venueCity?.trim() || formData.venuePinCode?.trim() ? (
          <iframe
            ...
            src={(() => {
              ...
              // Using OpenStreetMap direct embed:
              // 1. Completely removes the Google white card popup
              // 2. Zooms directly via normal mouse wheel scrolling without requiring Ctrl
              // 3. No API key needed
              return `https://www.openstreetmap.org/export/embed.html?layer=mapnik&q=${encodeURIComponent(query)}`;
            })()}
          ></iframe>
        ) : (
