...
useEffect(() => {
  ...
  if (location.state?.seatMapStats) {
    setSeatStats(location.state.seatMapStats); // <-- Stores the counts
  }
}, [location.state]);
...
// Add a time slot for a specific date
const addWeeklySlotForDate = (dateVal) => {
  setFormData(prev => ({
    ...prev,
    weeklyTimeSlots: [...(prev.weeklyTimeSlots || []), { date: dateVal, startTime: '', endTime: '' }]
  }));
};

// Delete a specific extra slot by index
const deleteWeeklySlotAtIndex = (globalIndex) => {
  setFormData(prev => ({
    ...prev,
    weeklyTimeSlots: (prev.weeklyTimeSlots || []).filter((_, i) => i !== globalIndex)
  }));
};

// Remove a whole date and all its associated slots
const removeEntireWeeklyDate = (dateVal) => {
  setFormData(prev => ({
    ...prev,
    selectedWeeklyDates: (prev.selectedWeeklyDates || []).filter(d => d !== dateVal),
    weeklyTimeSlots: (prev.weeklyTimeSlots || []).filter(s => s.date !== dateVal)
  }));
};
  ...
  const buildPayload = () => ({
    ...
    venue: {
      ...
      googleMapLink: formData.venueGoogleMapLink || (
        [formData.venueName, formData.venueAddress, formData.venueCity, formData.venuePinCode].filter(Boolean).length > 0
          ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([formData.venueName, formData.venueAddress, formData.venueCity, formData.venuePinCode].filter(Boolean).join(', '))}`
          : ''
      )
    },
...
{/* Removable Date Pills: Top-right overlapping badge on hover */}
              ...
              {formData.selectedWeeklyDates.map((dateStr, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Floating Top-Right Badge: appears only on hover */}
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="absolute -top-2 -right-2 w-4 h-4 rounded-full bg-rose-100 text-rose-500 hover:bg-rose-200 hover:text-rose-700 flex items-center justify-center font-bold text-xs cursor-pointer shadow-sm opacity-0 group-hover:opacity-100 transition-all duration-150 transform scale-90 group-hover:scale-100"
                      title="Remove date"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5 stroke-2">
        ...
        <div className="space-y-4 w-full pt-2">
          ...
          {formData.sameTimeSlotForAll ? (
            ...
            /* SEPARATE DATE-WISE BOX CARDS */
            (formData.selectedWeeklyDates || []).map((dateVal) => {
              const d = new Date(dateVal);
              const dayNum = d.getDate();
              const monthStr = d.toLocaleDateString('en-US', { month: 'short' });
              const weekdayStr = d.toLocaleDateString('en-US', { weekday: 'short' });
              const dateHeaderTitle = `${weekdayStr} ${dayNum} ${monthStr}`;

              const dateSlotsWithIndex = (formData.weeklyTimeSlots || [])
                .map((slot, globalIndex) => ({ ...slot, globalIndex }))
                .filter(slot => slot.date === dateVal);
                  ...
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                    ...
                    <button
                      ...
                      onClick={() => {
                        setFormData({
                          ...formData,
                          selectedWeeklyDates: (formData.selectedWeeklyDates || []).filter(d => d !== dateVal),
                          weeklyTimeSlots: (formData.weeklyTimeSlots || []).filter(s => s.date !== dateVal)
                        });
                      }}
                      className="text-slate-400 hover:text-rose-600 transition p-1 cursor-pointer"
                      title={`Delete ${dateHeaderTitle}`}
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-4 h-4">
                        <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                      </svg>
...
<div className="flex flex-col lg:flex-row gap-8 items-start justify-center pb-24">
  {/* Left Column: Fixed 600x750 Frame */}
  <div className="flex flex-col items-center">
    
    {/* OUTSIDE TOP BAR: ZOOM IN / OUT */}
    {seatMapImage && (
      <div className="w-full max-w-[600px] flex items-center justify-between pb-2 px-1">
        <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
          Layout Canvas (600 × 750)
        </span>
        <div className="flex items-center space-x-1.5 bg-slate-100 border border-slate-200 rounded-lg p-0.5 shadow-xs">
          <button
            type="button"
            onClick={() => setImgZoom((prev) => Math.max(0.6, Number((prev - 0.2).toFixed(2))))}
            className="w-6 h-6 flex items-center justify-center rounded bg-white hover:bg-slate-200 border border-slate-300 cursor-pointer text-slate-800 font-extrabold text-xs shadow-xs"
            title="Zoom Out"
          >
            -
          </button>
          <span className="w-10 text-center font-bold text-slate-700 text-[11px]">
    ...
    {/* EXACT 600 x 750 BOX WITH OBJECT-COVER AND DRAG/ZOOM */}
    <div
      ...
      className={`border-2 border-slate-300 rounded-2xl relative flex items-center justify-center overflow-hidden transition bg-slate-900 shadow-xl shrink-0 ${
        seatMapImage
          ? isDraggingImg
    ...
    >
      {seatMapImage ? (
        ...
        <div className="p-8 text-center text-slate-400">
          ...
          <span className="text-sm font-semibold block">600 × 750 Seating Canvas</span>
          <span className="text-xs text-slate-500 mt-1 block">Drag & drop image or use builder</span>
        </div>
      )}
    </div>
  </div>
  ...
  <div className="flex flex-col gap-3 w-full max-w-sm pt-4">
    <Link 
      to="/seatmap" 
      className="py-3.5 px-4 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-xl text-center no-underline shadow-md transition"
    >
    ...
    <label className="py-3.5 px-4 bg-white border-2 border-slate-200 hover:border-slate-300 text-slate-800 text-xs font-bold rounded-xl text-center cursor-pointer shadow-xs transition">
      Upload 600×750 image
      <input 
        type="file" 
        accept="image/*" 
        onChange={(e) => {
    ...
    <div className="p-3 bg-blue-50/60 border border-blue-100 rounded-xl flex items-start gap-2.5 mt-2">
      ...
      <p className="text-[11px] text-slate-600 leading-relaxed">
        The canvas area is locked to <strong>600 × 750px</strong>. Images will cover the area cleanly while preserving your interactive pan/zoom navigation.
      </p>
    </div>
  </div>
</div>
...
<div className="space-y-4 pt-4">
  ...
  <div className="border-b border-slate-100"></div>
      ...
      <div className="flex justify-between items-center py-2 ">
        ...
        {/* FIXED BOTTOM FOOTER: SHOWS ALL SEAT DATA */}
{seatStats && (
  <footer className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 h-14 px-6 flex items-center justify-between z-50 shadow-lg text-xs">
    {/* Left Side: Core Totals */}
    <div className="flex items-center space-x-6 text-sm">
      <div className="flex flex-col items-center">
        <span className="font-extrabold text-slate-900 text-base">{seatStats.totalSeats || 0}</span>
        <span className="text-[10px] text-slate-500 font-medium uppercase">Total Seats</span>
      </div>
      <div className="h-6 w-[1px] bg-slate-200" />
      <div className="flex flex-col items-center">
        <span className="font-extrabold text-emerald-600 text-base">{seatStats.availableSeats || 0}</span>
        <span className="text-[10px] text-slate-500 font-medium uppercase">Available</span>
      </div>
      <div className="h-6 w-[1px] bg-slate-200" />
      <div className="flex flex-col items-center">
        <span className="font-extrabold text-amber-600 text-base">{seatStats.reservedSeats || 0}</span>
        <span className="text-[10px] text-slate-500 font-medium uppercase">Reserved</span>
      </div>
    </div>

    {/* Center / Right Side: Category Badges (VIP, Regular, etc.) */}
    <div className="flex items-center space-x-2 overflow-x-auto py-1">
      {Object.entries(seatStats.categoryCounts || {}).map(([catName, count]) => (
        <div 
          key={catName}
          className="flex items-center space-x-1.5 px-3 py-1 rounded-full bg-slate-100 border border-slate-200 text-slate-700 shadow-2xs"
        >
          <span className="w-2.5 h-2.5 rounded-full bg-indigo-500" />
          <span className="font-semibold text-xs">{catName}:</span>
