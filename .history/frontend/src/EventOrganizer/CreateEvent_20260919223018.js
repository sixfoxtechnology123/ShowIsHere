...
<div className="space-y-2 w-full">
  ...
  <h3 className="text-xl font-bold text-slate-900 mb-6">Seat Map configuration</h3>

  <div className="flex flex-col lg:flex-row items-start gap-12">
    {/* Left Column: Fixed Preview Box & Info Box */}
    <div className="w-[420px] shrink-0 space-y-3">
      {/* Box strictly sized to match image dimensions */}
      <div
        ...
        className={`w-[420px] h-[250px] rounded-2xl border-2 border-slate-300 bg-white flex items-center justify-center overflow-hidden transition relative ${
          seatMapImage
            ? isDraggingImg
      ...
      {/* Info Notice Box */}
      <div className="w-[420px] p-3 bg-blue-50/70 border border-blue-100 rounded-xl flex items-center gap-3">
        ...
        <p className="text-[11px] text-slate-600 leading-snug">
          Upload a image of your event seat map map as per your ticket category (.jpg or .png 600 X 750px recomended)[cite: 3]
        </p>
      </div>
    </div>

    {/* Right Column: Exact Buttons Layout */}
    <div className="w-[260px] flex flex-col items-center gap-3 pt-6">
      <Link 
        to="/seatmap" 
        className="w-full py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-xl text-center no-underline shadow-xs transition"
      >
        Open Seat Map Creator[cite: 3]
      </Link>
      
      <div className="text-center text-xs text-slate-500 font-medium my-0.5">or[cite: 3]</div>

      <label className="w-full py-3 px-4 bg-white border-2 border-slate-300 hover:border-slate-400 text-slate-800 text-sm font-semibold rounded-xl text-center cursor-pointer shadow-xs transition block">
        Upload image[cite: 3]
        <input 
          type="file" 
          accept="image/*" 
