...
<div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
  {/* Left Column: 600x750 Proportional Card with Attached Image Footer */}
  <div className="lg:col-span-7 flex flex-col items-center">
    
    {/* TOP BAR: ZOOM IN / OUT OUTSIDE THE BOX */}
    {seatMapImage && (
      <div className="w-full max-w-[420px] flex items-center justify-between pb-2 px-1">
    ...
    {/* 600 x 750 RATIO CONTAINER WITH ATTACHED FOOTER */}
    <div className="w-full max-w-[420px] border-2 border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm flex flex-col">
      ...
      <div
        ...
        className={`w-full aspect-[600/750] relative flex items-center justify-center overflow-hidden bg-slate-100 ${
          seatMapImage
            ? isDraggingImg
      ...
      >
        {seatMapImage ? (
          ...
          <div className="p-6 text-center text-slate-400">
            ...
            <span className="text-xs font-semibold block text-slate-600">600 × 750 Seating Canvas</span>
            <span className="text-[11px] text-slate-400 mt-0.5 block">Drag & drop image or use builder</span>
          </div>
        )}
      </div>

      {/* ATTACHED IMAGE FOOTER (TOTAL, AVAILABLE, RESERVED, VIP, REGULAR, ETC.) */}
      {seatStats ? (
        <div className="bg-white border-t border-slate-200 p-2.5 flex flex-col gap-2">
          {/* Main Counters Row */}
          <div className="flex items-center justify-between text-xs px-1">
            <div className="flex items-center space-x-4">
              <div className="text-center">
                <span className="font-extrabold text-slate-900 text-sm block leading-none">{seatStats.totalSeats || 0}</span>
                <span className="text-[9px] text-slate-500 font-medium uppercase">Total</span>
              </div>
              <div className="h-5 w-[1px] bg-slate-200" />
              <div className="text-center">
                <span className="font-extrabold text-emerald-600 text-sm block leading-none">{seatStats.availableSeats || 0}</span>
                <span className="text-[9px] text-slate-500 font-medium uppercase">Available</span>
              </div>
              <div className="h-5 w-[1px] bg-slate-200" />
              <div className="text-center">
                <span className="font-extrabold text-amber-600 text-sm block leading-none">{seatStats.reservedSeats || 0}</span>
                <span className="text-[9px] text-slate-500 font-medium uppercase">Reserved</span>
              </div>
            </div>
          </div>

          {/* Dynamic Category Badges Row (VIP, Regular, Premium, etc.) */}
          {seatStats.categoryCounts && Object.keys(seatStats.categoryCounts).length > 0 && (
            <div className="flex items-center flex-wrap gap-1.5 pt-1.5 border-t border-slate-100">
        ...
        <div className="bg-slate-50 border-t border-slate-200 py-2 px-3 text-[11px] text-slate-400 text-center font-medium">
          Seat details will appear here once saved from creator
        </div>
      )}
    </div>
  </div>
