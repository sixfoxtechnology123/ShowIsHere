...
const handleDragOver = (e) => {
  ...
  // Helper to collect all current form data cleanly
  const buildPayload = () => ({
  ...
  // 1. "Save as Draft" Button -> Calls MongoDB
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      setIsDataSaved(true);
      toast.success('Draft saved in database!');
    } catch (err) {
  ...
  // 2. "Next / Proceed" Button -> Saves step to MongoDB before changing step
...
const handleProceed = async () => {
  ...
  // 1. Save data to database on every step forward
  try {
    const payload = {
      eventId: createdEventId,
      currentActiveStep: activeStep,
    ...
    if (activeStep < steps.length) {
      // Steps 1 to 5: Save draft step
      const res = await API.post('/events/save-step', payload);
      const saved = res.data?.data || res.data || res;
      if (saved?._id) setCreatedEventId(saved._id);
      ...
      toast.success('Event published and stored in database successfully!');
    }
  } catch (err) {
    toast.error(err.response?.data?.message || 'Failed to save event to database.');
  }
};
