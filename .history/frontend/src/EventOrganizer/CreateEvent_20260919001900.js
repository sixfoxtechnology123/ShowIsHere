...
const extractEmbedSrcFromLink = (link) => {
  if (!link) return null;
  const trimmed = link.trim();

  // 1. Direct coordinates: "22.5726, 88.3639"
  if (/^-?\d+(\.\d+)?,\s*-?\d+(\.\d+)?$/.test(trimmed)) {
    return `https://maps.google.com/maps?q=${encodeURIComponent(trimmed)}&t=m&z=17&output=embed`;
  }

  // 2. Short links (maps.app.goo.gl or goo.gl) - load clean query without tracking params
  if (trimmed.includes('goo.gl') || trimmed.includes('maps.app.goo.gl')) {
    const cleanUrl = trimmed.split('?')[0];
    return `https://maps.google.com/maps?q=${encodeURIComponent(cleanUrl)}&t=m&z=16&output=embed`;
  }

  try {
    const parsed = new URL(trimmed);

    // 3. URLs with ?q= parameter
    const qParam = parsed.searchParams.get('q');
    if (qParam) {
      return `https://maps.google.com/maps?q=${encodeURIComponent(qParam)}&t=m&z=17&output=embed`;
    }

    // 4. URLs with /place/Name/@lat,lng
    if (trimmed.includes('/place/')) {
      const match = trimmed.match(/\/place\/([^/@?]+)/);
      if (match && match[1]) {
        const place = decodeURIComponent(match[1].replace(/\+/g, ' '));
        return `https://maps.google.com/maps?q=${encodeURIComponent(place)}&t=m&z=17&output=embed`;
      }
    }
    ...
    if (coordMatch) {
      return `https://maps.google.com/maps?q=${coordMatch[1]},${coordMatch[2]}&t=m&z=17&output=embed`;
    }
  } catch (err) {
    // Plain string fallback
  }

  return `https://maps.google.com/maps?q=${encodeURIComponent(trimmed)}&t=m&z=16&output=embed`;
};
...
<div>
  ...
  <input 
    ...
    onChange={async (e) => {
      ...
      // Resolve short links via the backend controller
      if (val.includes('goo.gl')) {
        try {
          const res = await API.get(`/resolve-map-link?url=${encodeURIComponent(val.trim())}`);
          if (res.data?.embedUrl) {
            setResolvedEmbedUrl(res.data.embedUrl);
          }
        } catch (err) {
  ...
  type="button"
  onClick={() => {
    setResolvedEmbedUrl(''); // <-- Add this line
    setFormData(prev => ({
      ...prev,
      venueName: '',
      venueAddress: '',
...
<iframe
  ...
  loading="lazy"
  src={(() => {
    // 1. If backend resolved a short link (goo.gl / maps.app.goo.gl)
    if (resolvedEmbedUrl) {
      return resolvedEmbedUrl;
    }

    // 2. Direct GPS coordinates or full standard Google URL
    if (formData.venueGoogleMapLink?.trim() && !formData.venueGoogleMapLink.includes('goo.gl')) {
      return `https://maps.google.com/maps?q=${encodeURIComponent(formData.venueGoogleMapLink.trim())}&t=m&z=17&output=embed`;
    }

    // 3. Fallback: Search using typed fields (e.g. SBI, Address, City, PIN)
    const queryParts = [
    ...
    // Dynamic zoom: SBI only = z11 (multiple red pins), with area/pin = z14/z17
    let zoomLevel = 11;
    if (formData.venuePinCode || (formData.venueAddress && formData.venueCity)) {
      zoomLevel = 17;
    } else if (formData.venueAddress || formData.venueCity) {
      zoomLevel = 14;
    }

    return `https://maps.google.com/maps?q=${encodeURIComponent(query)}&t=m&z=${zoomLevel}&output=embed`;
  })()}
></iframe>
