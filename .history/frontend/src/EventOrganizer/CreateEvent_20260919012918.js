...
const [mapType, setMapType] = useState('m'); // 'm' = street view, 'k' = satellite
...
{/* Google Map View with Preserved Satellite & Zoom */}
<div className="space-y-2 w-full">
  <div className="flex items-center justify-between">
    <label className={accountLabelStyle}>Location Map</label>
    <span className="text-[11px] text-slate-400">
      Current view: <strong className="text-slate-700">{mapType === 'm' ? 'Street View' : 'Satellite View'}</strong>
    </span>
  </div>
      ...
      <>
        <iframe
          ...
          src={(() => {
            ...
            // &t=${mapType} ensures Satellite ('k') or Street ('m') stays locked when zooming
            return `https://maps.google.com/maps?q=${encodeURIComponent(query)}&z=${mapZoom}&t=${mapType}&iwloc=near&output=embed`;
          })()}
        ></iframe>

        {/* Floating Controls: Switch Mode + Zoom */}
        <div className="absolute right-3 top-3 flex flex-col shadow-md rounded-xl overflow-hidden z-20 bg-white border border-slate-200">
          {/* Satellite / Street Switcher Button */}
          <button
            type="button"
            onClick={() => setMapType((prev) => (prev === 'm' ? 'k' : 'm'))}
            className="w-10 h-10 bg-white hover:bg-slate-50 flex items-center justify-center text-xs font-bold text-slate-700 border-b border-slate-100 cursor-pointer transition select-none"
            title={mapType === 'm' ? 'Switch to Satellite View' : 'Switch to Street View'}
          >
            {mapType === 'm' ? '🛰️' : '🗺️'}
          </button>
          ...
          <button
            ...
            className="w-10 h-9 bg-white hover:bg-slate-50 flex items-center justify-center text-slate-800 font-bold text-lg border-b border-slate-100 cursor-pointer transition select-none"
            title="Zoom in"
          >
            +
          </button>
          ...
          <button
            ...
            className="w-10 h-9 bg-white hover:bg-slate-50 flex items-center justify-center text-slate-800 font-bold text-lg cursor-pointer transition select-none"
            title="Zoom out"
          >
            −
          </button>
