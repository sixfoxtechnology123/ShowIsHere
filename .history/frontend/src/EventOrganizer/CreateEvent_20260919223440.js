...
<div className="space-y-2 w-full">
  ...
  <div className="flex flex-col lg:flex-row items-start gap-12 pt-2">
    {/* Left Column: Fixed Width Matching Screenshot */}
    <div className="w-[380px] shrink-0 space-y-3">
      
      {/* Box strictly locked to the screenshot aspect ratio */}
      <div
        ...
        className={`w-[380px] h-[215px] rounded-2xl border-2 border-slate-300 bg-white flex items-center justify-center overflow-hidden relative ${
          seatMapImage
            ? isDraggingImg
      ...
      {/* Info Notice Box */}
      <div className="w-[380px] p-2.5 bg-blue-50/70 border border-blue-100 rounded-xl flex items-center gap-2.5">
        ...
        <p className="text-[10px] text-slate-600 leading-snug">
          Upload a image of your event seat map map as per your ticket category (.jpg or .png 600 X 750px recomended)[cite: 4]
        </p>
      </div>
    </div>

    {/* Right Column: Exact Compact Button Sizing */}
    <div className="w-[210px] flex flex-col items-center gap-2.5 pt-4">
      <Link 
        to="/seatmap" 
        className="w-full py-2.5 px-4 bg-[#1E60F2] hover:bg-blue-700 text-white text-xs font-semibold rounded-xl text-center no-underline shadow-xs transition"
      >
        Open Seat Map Creator[cite: 4]
      </Link>
      
      <div className="text-center text-xs text-slate-500 font-medium my-0.5">or[cite: 4]</div>

      <label className="w-full py-2.5 px-4 bg-white border border-slate-300 hover:border-slate-400 text-slate-800 text-xs font-semibold rounded-xl text-center cursor-pointer shadow-xs transition block">
        Upload image[cite: 4]
        <input 
          type="file" 
          accept="image/*" 
