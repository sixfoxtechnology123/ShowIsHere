...
// Reliable Red Pin icon that does not require external PNG assets
const customRedMarkerIcon = L.divIcon({
  className: 'custom-map-pin',
  html: `
    <div style="position: relative; width: 30px; height: 42px;">
      <svg viewBox="0 0 24 36" fill="none" xmlns="http://www.w3.org/2000/svg" style="width: 100%; height: 100%; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.35));">
...
const [mapCoords, setMapCoords] = useState({ lat: 22.5726, lng: 88.3639 });
...
// Helper 1: Recenter map and fix grey-tile rendering on step changes
...
// Helper 2: Click anywhere to move marker
  ...
  const timer = setTimeout(async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        setFormData(prev => ({
          ...prev,
          venueGoogleMapLink: `${newCoords.lat},${newCoords.lng}`
        }));
      }
    } catch (err) {
...
}, [formData.venueName, formData.venueAddress, formData.venueCity, formData.venuePinCode]);
// Validation helpers
const isValidCity = (val) => /^[a-zA-Z\s.-]+$/.test(val.trim());
const isPinCode = (val) => /^[1-9][0-9]{5}$/.test(val.trim());
const containsStreetOrAddress = (val) => /\b(road|rd|street|st|lane|ave|avenue|sarani|nagar|colony|block|plot|floor|near|opp|behind|beside)\b/i.test(val) || /\d+[\/-]\w+/.test(val);
...
<div className="pt-6 border-t border-slate-100 space-y-4 w-full">
  ...
  <div className="flex flex-col gap-6 w-full">
    ...
    <div className="space-y-4 w-full">
      ...
      <div>
        ...
        <input 
          type="text" 
          name="venueName" 
          placeholder="e.g. State Bank of India, Mahajati Sadan" 
          value={formData.venueName || ''} 
          onChange={(e) => {
            // Block entering pure PIN code numbers into Venue Name
            if (/^\d{6}$/.test(e.target.value.trim())) return;
            handleInputChange(e);
          }} 
          className={`${inputFieldStyle} border-2 w-full`} 
        />
      ...
      <div>
        ...
        <input 
          ...
          onChange={(e) => {
            // Block entering pure PIN code numbers into Address
            if (/^\d{6}$/.test(e.target.value.trim())) return;
            handleInputChange(e);
          }} 
          className={`${inputFieldStyle} border-2 w-full`} 
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input 
            ...
            onChange={(e) => {
              // Allows only letter characters, spaces, and hyphens
              const val = e.target.value;
        ...
        <div>
          ...
          <input 
            ...
            onChange={(e) => {
              // Strictly digits only (max 6 numbers). Completely blocks text/venue names.
    ...
    {/* Reset Location Action (Silent - No Toasts) */}
    <div>
      <button
        type="button"
        onClick={() => {
...
<div className="space-y-2 w-full">
    ...
    {formData.venueName?.trim() || formData.venueAddress?.trim() || formData.venueCity?.trim() || formData.venuePinCode?.trim() ? (
      <iframe
        ...
        src={(() => {
          ...
          // Join fields into a clean search query
          const query = [name, address, city, pin].filter(Boolean).join(', ');

          // Dynamic zoom level:
          // Just name (e.g. 'SBI') -> z=11 (wide view with multiple red pin markers)
          // With Address -> z=14
          // With City or PIN -> z=16 (exact street pinpoint)
          let zoom = 11;
          if (pin || (address && city)) {
            zoom = 16;
          ...
          // &iwloc=near suppresses the floating white popup card
          return `https://maps.google.com/maps?q=${encodeURIComponent(query)}&z=${zoom}&t=m&iwloc=near&output=embed`;
        })()}
      ></iframe>
    ) : (
  ...
  <div className="border-2 border-slate-200 rounded-2xl h-96 bg-slate-100 relative overflow-hidden shadow-xs w-full z-0">
    <MapContainer
    ...
    >
      ...
      <MapRecenter center={mapCoords} />

      <LocationPicker
        onLocationSelect={(lat, lng) => {
          setMapCoords({ lat, lng });
          setFormData(prev => ({ ...prev, venueGoogleMapLink: `${lat},${lng}` }));
        }}
      />
...
<Marker
  ...
  eventHandlers={{
    dragend: (e) => {
      ...
      setFormData(prev => ({ ...prev, venueGoogleMapLink: `${pos.lat},${pos.lng}` }));
    }
  }}
/>
    </MapContainer>
