...
<div className="space-y-5 pt-2 w-full">
        ...
        {formData.selectedWeeklyDates.map((dateStr, i) => {
          ...
          return (
            <div key={i} className="relative group px-4 py-2 bg-blue-600 text-white rounded-md text-xs font-bold shadow-xs inline-flex items-center justify-center">
              ...
              <button
                type="button"
                onClick={() => removeEntireWeeklyDate(dateStr)}
  ...
  {/* Same Time Slot For All Days Checkbox */}
  ...
  <div className="space-y-4 w-full pt-2">
    ...
    {/* CASE A: Same time slot for all dates */}
    {formData.sameTimeSlotForAll ? (
            ...
            <div key={index} className="flex items-center gap-3 w-full">
              ...
              <div className="mt-5 flex items-center gap-1.5">
                {!isDefault && (
                  <button 
                    type="button" 
                    onClick={() => deleteWeeklySlotAtIndex(index)} 
      ...
      /* CASE B: Separate Date-Wise Box Cards */
      (formData.selectedWeeklyDates || []).map((dateVal) => {
        ...
        return (
          <div key={dateVal} className="border border-slate-200 rounded-xl p-4 bg-white shadow-2xs space-y-3">
            ...
            {/* Time Slot Rows for this Date */}
            <div className="space-y-3">
              {dateSlotsWithIndex.map((slotItem, localIndex) => {
                const isDefaultRow = localIndex === 0;
                const isLastRow = localIndex === totalForThisDate - 1;
                ...
                return (
                  <div key={slotItem.globalIndex} className="flex items-center gap-3 w-full">
                    ...
                    {/* Action Buttons */}
                    <div className="mt-5 flex items-center gap-1.5">
                      {/* Trash Delete Icon: Hidden on the first/default row, visible on any additional row */}
                      {!isDefaultRow && (
                        <button 
                          type="button" 
                          onClick={() => deleteWeeklySlotAtIndex(slotItem.globalIndex)} 
                          className="w-9 h-9 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-100 flex items-center justify-center cursor-pointer transition"
                          title="Delete added row"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-4 h-4">
                            <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                          </svg>
                      ...
                      {isLastRow && (
                        <button 
                          type="button" 
                          onClick={() => addWeeklySlotForDate(dateVal)} 
                          className="w-9 h-9 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center justify-center font-bold text-base cursor-pointer transition"
                          title="Add time slot"
                        >
                          +
