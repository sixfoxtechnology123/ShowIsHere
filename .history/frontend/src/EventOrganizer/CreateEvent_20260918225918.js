...
// 1. "Save as Draft" Button
  const handleSaveDraft = async () => {
    ...
    setIsSaving(true);
    const toastId = toast.loading('Saving draft with images to database...');
    ...
    try {
      ...
      setIsDataSaved(true);
      toast.success(res.data?.message || 'Draft saved successfully!', { id: toastId });
    } catch (err) {
      console.error('Draft error:', err);
      toast.error(err.response?.data?.message || 'Failed to save draft.', { id: toastId });
    } finally {
      setIsSaving(false);
    }
  };

  // 2. "Next / Proceed" Button
  const handleProceed = async () => {
    ...
    const toastId = toast.loading('Saving step data...');
    ...
    try {
      ...
      if (activeStep < steps.length) {
        ...
        toast.success(res.data?.message || `Step ${activeStep} saved!`, { id: toastId });
        ...
        toast.success(res.data?.message || 'Event published successfully!', { id: toastId });
      }
    } catch (err) {
      console.error('Proceed error:', err);
      toast.error(err.response?.data?.message || 'Failed to proceed.', { id: toastId });
    }
  };
