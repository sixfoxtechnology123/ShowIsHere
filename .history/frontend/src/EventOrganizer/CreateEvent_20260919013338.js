...
{/* Location Map with Native Google Controls (Preserves Satellite Mode) */}
    ...
    {formData.venueName?.trim() || formData.venueAddress?.trim() || formData.venueCity?.trim() || formData.venuePinCode?.trim() ? (
      <iframe
        ...
        src={(() => {
          ...
          // Default initial zoom when user finishes typing
          let defaultZoom = 12;
          if (pin || (address && city)) {
            defaultZoom = 16;
          ...
          return `https://maps.google.com/maps?q=${encodeURIComponent(query)}&z=${defaultZoom}&iwloc=near&output=embed`;
        })()}
      ></iframe>
    ) : (
