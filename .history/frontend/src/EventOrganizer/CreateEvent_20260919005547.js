...
// Helper 1: Recenter map when position changes
...
useEffect(() => {
  ...
  }, 800); // 800ms debounce to avoid rapid calls while typing
...
<div className="pt-6 border-t border-slate-100 space-y-4 w-full">
  ...
  <div className="flex flex-col gap-6 w-full">
    ...
    {/* Clean Map Container (Accurate Search, No Top-Left Details Card, Direct Scroll Zoom) */}
    <div className="space-y-2 w-full">
      <label className={accountLabelStyle}>Location Map</label>
      <div className="border-2 border-slate-200 rounded-2xl h-96 bg-slate-100 flex items-center justify-center text-center relative overflow-hidden shadow-xs w-full">
        {formData.venueName?.trim() || formData.venueAddress?.trim() || formData.venueCity?.trim() || formData.venuePinCode?.trim() ? (
          <iframe
            ...
            src={(() => {
              ...
              // Strict hierarchy search query
              const query = [name, address, city, pin].filter(Boolean).join(', ');

              // Dynamic zoom:
              // Venue only (e.g. "SBI") -> z=11 (wide view showing all red pin branches)
              // Venue + Address -> z=14 (locality area)
              // Venue + City/PIN -> z=16 (exact location)
              let zoom = 11;
              if (pin || (address && city)) {
                zoom = 16;
              ...
              // Using Google's clean static search mode:
              // &iwloc=suppress completely suppresses the white info popup card
              return `https://maps.google.com/maps?q=${encodeURIComponent(query)}&z=${zoom}&t=m&iwloc=suppress&output=embed`;
            })()}
          ></iframe>
        ) : (
          <div className="space-y-1 p-4">
            ...
            <p className="text-[11px] text-slate-400">Enter venue details above to view the location map</p>
          </div>
        )}
      </div>
    </div>
