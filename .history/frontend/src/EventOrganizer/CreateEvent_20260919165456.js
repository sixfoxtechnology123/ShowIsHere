...
<div className="space-y-2 w-full">
  ...
  <div className="lg:col-span-7 space-y-2.5">
    ...
    {/* OUTSIDE TOP BAR: ZOOM CONTROLS (ONLY OUTSIDE THE BOX) */}
    {seatMapImage && (
    ...
    {/* EXACT h-48 DRAGGABLE & ZOOMABLE IMAGE BOX (CLEAN, NO BUTTONS INSIDE) */}
    <div
      ...
      className={`border-2 border-slate-200 rounded-2xl h-48 relative flex items-center justify-center p-2 text-center overflow-hidden transition bg-slate-50 ${
        seatMapImage
          ? isDraggingImg
    ...
    >
      {seatMapImage ? (
        ...
        <span className="text-xs text-slate-400 font-medium pointer-events-none">
          Drag & drop seating image here or click Open Seat Map Creator
        </span>
      )}
    </div>

    {/* SEAT STATISTICS FOOTER BAR (SHOWS ALL SEAT DATA) */}
    {seatStats && (
      <div className="bg-white border border-slate-200 rounded-xl p-2.5 shadow-xs flex flex-wrap items-center justify-between gap-2 text-xs">
        <div className="flex items-center space-x-3">
          <div>
            ...
            <span className="font-extrabold text-slate-900 text-sm">{seatStats.totalSeats}</span>
          </div>
          <div className="h-6 w-[1px] bg-slate-200" />
          <div>
            <span className="text-[10px] text-slate-400 uppercase font-semibold block leading-tight">Available</span>
            <span className="font-extrabold text-emerald-600 text-sm">{seatStats.availableSeats}</span>
          </div>
          <div className="h-6 w-[1px] bg-slate-200" />
          <div>
            <span className="text-[10px] text-slate-400 uppercase font-semibold block leading-tight">Reserved</span>
            <span className="font-extrabold text-amber-600 text-sm">{seatStats.reservedSeats}</span>
          </div>
        </div>

        {/* Dynamic Category Badges (VIP, Regular, Premium, etc.) */}
        <div className="flex items-center flex-wrap gap-1.5 pl-2 border-l border-slate-200">
