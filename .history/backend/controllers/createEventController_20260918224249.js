...
// 2. Save Step Data or Draft (Generates CE1, CE2... and attaches tenantKey)
exports.saveEventStepData = async (req, res) => {
  try {
    ...
    if (!eventData.eventName || !eventData.eventCategoryId) {
      return res.status(400).json({ success: false, message: 'Event Title and Category are required' });
    }
    ...
    // Generate atomic CE1, CE2, CE3... format ID for the tenant
