...
// 6. Resolve Google Maps Short Links (Follow Redirects)
exports.resolveMapLink = async (req, res) => {
  try {
    ...
    // Follow redirect headers to get the final Google Maps URL
    const response = await fetch(url, { redirect: 'follow', method: 'GET' });
    const finalUrl = response.url;

    // 1. Check coordinates (@lat,lng)
