...
exports.saveEventStepData = async (req, res) => {
  try {
    ...
    // A. UPDATE EXISTING DRAFT (Do NOT generate a new createEventId)
    if (eventId && mongoose.Types.ObjectId.isValid(eventId)) {
      ...
      return res.status(200).json({
        success: true,
        data: updatedEvent,
        message: `Step ${currentActiveStep} updated successfully! (${updatedEvent.createEventId})`
      });
    }

    // B. NEW EVENT CREATION: Generate CE ID ONLY AFTER validation passes and right before save
    const createEventId = await getNextCreateEventId(tenantKey);
    ...
    return res.status(201).json({
      success: true,
      data: newEvent,
      message: `Draft created successfully with ID: ${createEventId}!`
    });
  } catch (error) {
    console.error('Save Draft DB Error:', error);

    // Handle MongoDB duplicate key index error gracefully
    if (error.code === 11000) {
      return res.status(409).json({
        success: false,
        message: 'Event ID collision occurred. Please click save again.'
      });
    }
